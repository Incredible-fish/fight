...
#fight库
fight_list = ["fight3.2.2","fight3.2.1","fight3.2.0","fight3.1.0","fight3.0.0","fight2.1.0","fight2.0.0","fight0.0.0"]
...
def automatic_update_find_code(url,Connect_timeout=3,Read_timeout=3):
    ...
    try:
        ...
        raise DownpassError(f"{url} It has expired and is waiting for the developer to upload it again")
...
def automatic_update():
    try:
        ...
        find_code = re.findall(re_txt,htmls,re.S)
        remote_code = find_code[0][17:len(find_code)-12]
        remote_code = html.unescape(remote_code)
        fihgt_all_req = json.loads(remote_code,strict = False)
